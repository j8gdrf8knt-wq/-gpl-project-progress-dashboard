      ...
      mutations.forEach(m => {
        if(m.attributeName === 'class'){
          ...
          if(overlay.classList.contains('open')){
            ...
            // Closing animation (symmetric exit)
            modal.style.transition = 'opacity .22s ease, transform .22s cubic-bezier(.34,1.3,.64,1)';
            modal.style.opacity = '0';
            modal.style.transform = 'translateY(-16px) scale(.97)';
          }
        }
...
// ── Crossfade for view/project switches ────────────────────────

function crossfadeMainContent(){
  const mc = document.getElementById('mainContent');
  if(!mc) return;
  mc.style.opacity = '0';
  mc.style.transition = 'opacity .2s ease';
  setTimeout(() => {
    mc.style.opacity = '1';
  }, 20);
}
...
function runPageAnimations(){
  // Only run full entrance animation on first load
  if(!_hasRunEntranceOnce){
    animateEntrance();
    animateCounters();
    animateTableRows();
    animateTitle(document.getElementById('tbTitle'));
    _hasRunEntranceOnce = true;
  } else {
    // On subsequent renders (view/project switches), just do a quick crossfade
    crossfadeMainContent();
  }
}
